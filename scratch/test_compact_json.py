...
2. For each item, output a JSON object with these EXACT short keys:
   - "c": Category name (e.g., "Sandwiches")
   - "n": Clean item name. Remove any prices, slashes, numbers, or "@" symbols from the name (e.g. "Bombay Cold Sandwich").
   - "p": Base price as a float. If multiple prices exist (e.g. '@40/@75'), use the first price (e.g., 40.0).
   - "d": Description. If multiple prices exist, specify details (e.g. "Half: 40 / Full: 75"). If single price, leave description empty.

Return ONLY a JSON array of these objects. Do not include explanations or markdown fences.
Format:
[
  {{
    "c": "Category Name",
...
try:
    print("Calling Qwen with compact JSON array...")
    ...
    with urllib.request.urlopen(req, timeout=120) as response:
        ...
        if isinstance(parsed, list):
            print(f"Parsed array with {len(parsed)} items.")
            print("First 3 items:")
            print(json.dumps(parsed[:3], indent=2))
        else:
            print("Parsed is dict, keys:", list(parsed.keys()))
