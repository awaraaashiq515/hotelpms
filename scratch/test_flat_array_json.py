...
prompt = f"""
Convert the raw OCR text from a restaurant menu image into a category-grouped flat JSON array.
...
Instructions:
1. Group and extract all items from the raw text.
2. Format each item strictly as a JSON array with 4 elements:
   [Category, Clean Item Name, Price, Description/Alternate Prices]
3. Clean names:
   - Extract only the name (e.g., "Bombay Cold Sandwich").
   - NEVER include price options, slashes, numbers, or "@" symbols in the item name.
4. Extract Prices:
   - Price must be a float of the first/base price option (e.g. 40.0 or 100.0).
   - If multiple price options exist, specify details in the description (e.g. "Half: 40 / Full: 75"). If single price, leave description empty.

Return ONLY a JSON array containing these item arrays. Do not include explanations.
Format:
[
  ["Sandwiches", "Bombay Cold Sandwich", 40.0, "Half: 40 / Full: 75"],
  ["Fries", "French Fries", 70.0, ""]
]
"""
...
try:
    print("Calling Qwen with flat array JSON...")
    ...
    with urllib.request.urlopen(req, timeout=120) as response:
        ...
        if isinstance(parsed, list):
            ...
            # Print a snippet of the dict values
            key = list(parsed.keys())[0]
            print(f"Sample values for {key}:")
            print(json.dumps(parsed[key][:3], indent=2))
