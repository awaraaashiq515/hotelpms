...
prompt = f"""
Convert the raw OCR text from a restaurant menu image into a clean, category-grouped JSON menu.
...
2. For each item:
   - "name": Clean name of the item. NEVER include price options, slashes, numbers, or "@" symbols in the name.
   - "price": Price as a float. If multiple options are present (e.g., '@40/@75' representing Half/Full), use the first/base price option (e.g. 40.0).
   - "description": Clean description. Include any pricing variations (e.g. "Half: 40 / Full: 75").

Return ONLY a JSON object matching this schema. Do not include markdown formatting or explanations.
{{
...
try:
    print("Calling Qwen JSON endpoint with no timeout...")
