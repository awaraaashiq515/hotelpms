...
// ── COMPONENT ──────────────────────────────────────────────────────────────
export default function CafePosView({
  terminalMode = 'CAFE',
}: {
    ...
    (async () => {
      ...
      try {
        ...
        if (propData.success) {
          ...
          setBarPosEnabled(!!propData.data.barPosEnabled);
      ...
      } catch { addToast('error', `Failed to load ${terminalMode === 'RESTAURANT' ? 'Restaurant' : terminalMode === 'BAR' ? 'Bar' : 'Cafe'} POS data`); }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (data.success) {
        addToast('success', `${terminalMode === 'RESTAURANT' ? 'Restaurant' : terminalMode === 'BAR' ? 'Bar' : 'Cafe'} order saved! Token: ${currentToken}`);
        clearCart();
        if (tableId) {
  ...
  if (loading || cafeEnabled === null) return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '18px', background: '#0E0A06' }}>
      ...
      <p style={{ fontSize: '11px', letterSpacing: '0.2em', color: 'rgba(255,255,255,0.25)', fontWeight: 600, textTransform: 'uppercase' }}>
        {terminalMode === 'RESTAURANT' ? 'Loading Restaurant POS...' : terminalMode === 'BAR' ? 'Pouring Bar POS...' : 'Brewing Cafe POS...'}
      </p>
    </div>
  );
    ...
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '28px', background: '#0E0A06' }}>
      ...
      <div style={{ textAlign: 'center' }}>
        <h2 style={{ fontSize: '22px', fontWeight: 700, color: '#FFFFFF', marginBottom: '10px', letterSpacing: '-0.02em' }}>
          {terminalMode === 'RESTAURANT' ? 'Restaurant' : terminalMode === 'BAR' ? 'Bar' : 'Cafe'} POS Disabled
        </h2>
        <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.3)', maxWidth: '340px', lineHeight: 1.7 }}>
          {terminalMode === 'RESTAURANT' ? 'Restaurant' : terminalMode === 'BAR' ? 'Bar' : 'Cafe'} POS module is currently turned off. Enable it from Settings.
        </p>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 16px', background: 'rgba(255,255,255,0.025)', borderBottom: '1px solid rgba(255,255,255,0.07)', backdropFilter: 'blur(24px)', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '9px', flexShrink: 0 }}>
            ...
            <div>
              <div style={{ fontSize: '14px', fontWeight: 800, color: '#D4956A', letterSpacing: '0.06em', lineHeight: 1 }}>
          ...
          <div style={{ flex: 1, maxWidth: '400px', position: 'relative' }}>
            ...
            <input
              type="text" placeholder={terminalMode === 'RESTAURANT' ? 'Search menu items...' : terminalMode === 'BAR' ? 'Search drinks, spirits...' : 'Search drinks, snacks...'}
