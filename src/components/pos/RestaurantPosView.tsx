...
const BAR_ACCENTS = [
  { color: '#E8A838', bg: 'rgba(232,168,56,0.10)', border: 'rgba(232,168,56,0.28)', glow: 'rgba(232,168,56,0.16)' },
...
const CAFE_CAT_ICON_MAP: Record<string, string> = {
...
const CAN_BE_HOT_COLD = ['latte', 'coffee', 'tea', 'espresso', 'mocha', 'cappuccino', 'macchiato', 'americano', 'milk', 'cocoa'];
...
export default function RestaurantPosView({
  terminalMode = 'RESTAURANT',
  themeLayout = 'RESTAURANT',
}: RestaurantPosViewProps) {
  ...
  const themeStyles = React.useMemo(() => {
    if (themeLayout === 'BAR') {
      return {
        '--pos-primary': '#E8A838',
        '--pos-primary-light': '#F2C26F',
        '--pos-primary-dark': '#C6871A',
        '--pos-primary-glow': 'rgba(232, 168, 56, 0.4)',
      } as React.CSSProperties;
    }
    if (themeLayout === 'CAFE') {
      return {
        '--pos-primary': '#D4956A',
        '--pos-primary-light': '#E8BC9E',
        '--pos-primary-dark': '#AA6F46',
        '--pos-primary-glow': 'rgba(212, 149, 106, 0.4)',
      } as React.CSSProperties;
    }
    return {
      '--pos-primary': '#e8a0a0',
      '--pos-primary-light': '#f0c8c8',
      '--pos-primary-dark': '#c47878',
    ...
    try {
      ...
      if (data.success) {
        ...
        data.data.forEach((floor: any) => {
          // Filter floors based on current POS terminalMode (RESTAURANT, BAR, CAFE)
          const isMatch = floor.menuType === terminalMode || 
            (terminalMode === 'RESTAURANT' && (!floor.menuType || floor.menuType === 'RESTAURANT'));
          if (!isMatch) return;
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const filteredProducts = (pData || []).filter((p: any) => p.menuType === terminalMode || (terminalMode === 'RESTAURANT' && !p.menuType));
      const filteredCategories = (cData || []).filter((c: any) => c.menuType === terminalMode || (terminalMode === 'RESTAURANT' && !c.menuType));
      ...
      if (propData.success) {
        ...
        if (terminalMode === 'RESTAURANT' && pData.restaurantPosEnabled === false) {
          if (pData.cafePosEnabled) {
            router.replace(`${pathPrefix}/cafe-pos`);
            return;
          } else if (pData.barPosEnabled) {
            router.replace(`${pathPrefix}/bar-pos`);
            return;
          } else {
            router.replace(`${pathPrefix}/operations`);
            return;
          }
        } else if (terminalMode === 'BAR' && pData.barPosEnabled === false) {
          if (pData.restaurantPosEnabled !== false) {
            router.replace(`${pathPrefix}/billing`);
            return;
          } else if (pData.cafePosEnabled) {
            router.replace(`${pathPrefix}/cafe-pos`);
            return;
          } else {
            router.replace(`${pathPrefix}/operations`);
        ...
        } else if (terminalMode === 'CAFE' && pData.cafePosEnabled === false) {
          if (pData.restaurantPosEnabled !== false) {
            router.replace(`${pathPrefix}/billing`);
            return;
          } else if (pData.barPosEnabled) {
  ...
  const addToCart = (product: Product | any, size: string = 'Full', price?: number, isCombo: boolean = false) => {
    setCart(prev => {
      ...
      return [...prev, { 
        ...
        quantity: 1,
        isCombo: isCombo,
        temp: canToggleTemp(product.name) ? 'HOT' : undefined
      }];
    });
  };
  ...
  const toggleTemp = (cartItemId: string) => {
  ...
  const replaceCartItem = (oldCartItemId: string, newProduct: Product, size: string = 'Full', price?: number) => {
    ...
    setCart(prev => {
      ...
      return prev.map(i => i.cartItemId === oldCartItemId ? {
        ...
        temp: canToggleTemp(newProduct.name) ? 'HOT' : undefined
      } : i);
    });
  ...
  const currentSubCategories = selectedCategory !== 'all' && selectedCategory !== 'breakfast' && selectedCategory !== 'lunch' && selectedCategory !== 'dinner' && selectedCategory !== 'combos'
  ...
  const filteredProducts = products.filter(p => {
    ...
    } else if (themeLayout === 'CAFE' && selectedSubCategory && selectedSubCategory !== 'all') {
      matchesCategory = p.categoryId === selectedSubCategory;
    } else if (themeLayout === 'CAFE') {
      const pCat = categories.find(c => c.id === p.categoryId);
      const parentId = pCat?.parentId;
      matchesCategory = p.categoryId === selectedCategory || parentId === selectedCategory;
    } else {
      matchesCategory = p.categoryId === selectedCategory;
    }
    const matchesSearch = p.name.toLowerCase().includes(search.toLowerCase());
    const matchesMenuType = (p as any).menuType === terminalMode || (terminalMode === 'RESTAURANT' && !(p as any).menuType);
...
=========================`;
  ...
  return (
    <div style={themeStyles} className={`flex h-full w-full ${theme === 'dark' ? 'bg-[#111111] text-slate-200' : 'bg-[#fdf8f8] text-[#2d1515]'} overflow-hidden font-sans selection:bg-pos-primary/30 transition-colors duration-500`}>
      {/* CENTER - Product Grid (Dynamic Theme Layout) */}
      <div className={`flex-1 flex flex-col h-full overflow-hidden ${
        ...
        {themeLayout === 'BAR' ? (
          <>
            ...
            <div className="px-3 py-1.5 flex items-center justify-between gap-3 bg-white/[0.01] border-b border-white/[0.05] backdrop-blur-md flex-shrink-0 z-10">
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <div className="w-8 h-8 rounded-lg bg-[#E8A838]/10 border border-[#E8A838]/20 flex items-center justify-center shadow-[0_0_12px_rgba(232,168,56,0.1)]">
                    <Wine size={16} color="#E8A838" />
                  </div>
                  <div>
                    <h1 className="text-xs font-black text-[#E8A838] tracking-widest leading-none uppercase">
                      {terminalMode === 'RESTAURANT' ? 'Restaurant POS' : terminalMode === 'CAFE' ? 'Cafe POS' : 'Bar POS'}
                    </h1>
                    <p className="text-[7px] text-slate-500 tracking-wider font-semibold uppercase mt-0.5">
                      {terminalMode === 'RESTAURANT' ? 'Classic Dining' : terminalMode === 'CAFE' ? 'Quick Bites' : 'Premium Bar'}
                    </p>
                  </div>
                </div>

                {/* ── POS MODE SWITCHER ── */}
                {(((property?.restaurantPosEnabled !== false ? 1 : 0) + (property?.barPosEnabled ? 1 : 0) + (property?.cafePosEnabled ? 1 : 0)) > 1) && (
                  <div className="flex items-center gap-0.5 p-0.5 rounded-lg border bg-white/[0.02] border-white/[0.05] flex-shrink-0 ml-2">
                    {property?.restaurantPosEnabled !== false && (
                      <button
                        ...
                        className={`px-2 py-1 rounded text-[9px] font-black uppercase tracking-wider transition-all ${
                          terminalMode === 'RESTAURANT'
                            ? 'bg-[#E8A838]/20 border border-[#E8A838]/30 text-[#E8A838] shadow-sm'
                            : 'text-slate-400 hover:bg-[#E8A838]/10 hover:text-[#E8A838]'
                        }`}
                      >
                        🍽 Restaurant
                      </button>
              ...
              <div className="flex-1 flex items-center gap-2 max-w-xl">
                <div className="relative flex-1 group">
                  <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-[#E8A838] transition-colors" size={14} />
                  <input 
                    type="text" 
                    placeholder="Search drinks, spirits..."
                    value={search}
                    onChange={(e) => setSearch(e.target.value)}
                    className="w-full bg-[#12121E] border border-white/5 focus:border-[#E8A838]/40 pl-9 pr-3 py-1.5 rounded-xl outline-none text-slate-200 placeholder:text-slate-600 font-bold text-xs"
                  />
                </div>
            ...
            {/* BAR CATEGORY PILLS */}
            <div className="px-3 py-1.5 overflow-x-auto no-scrollbar flex gap-2 border-b border-white/[0.03] bg-white/[0.005]">
              <button 
                onClick={() => setSelectedCategory('all')} 
                className={`flex-shrink-0 px-4 py-1.5 rounded-full text-xs font-semibold cursor-pointer white-space-nowrap transition-all ${
                  selectedCategory === 'all' 
                    ? 'border border-[#E8A838]/40 bg-[#E8A838]/15 text-[#E8A838] shadow-[0_0_12px_rgba(232,168,56,0.08)]' 
                    : 'border border-white/5 bg-[#12121E] text-slate-400 hover:text-white'
                }`}
              >
                All Items
              </button>
              ...
              {categories.map((cat, idx) => {
                ...
                return (
                  <button 
                    ...
                    style={{
                      borderColor: isSelected ? accent.border : 'rgba(255,255,255,0.05)',
                      backgroundColor: isSelected ? accent.bg : '#12121E',
                      color: isSelected ? accent.color : 'rgba(255,255,255,0.4)',
                    }}
                    className="flex-shrink-0 display-flex items-center gap-1.5 px-4 py-1.5 rounded-full text-xs font-semibold cursor-pointer white-space-nowrap transition-all border"
                  >
                    <span>{emoji}</span>
                    <span>{cat.name}</span>
                  </button>
            ...
            {/* BAR PRODUCTS GRID / LIST */}
            <div className="flex-1 overflow-y-auto p-3 scrollbar-thin">
              {filteredProducts.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center gap-3 opacity-20 py-12">
                  <FlaskConical size={44} className="text-slate-500" />
                  <p className="text-[11px] text-slate-400 font-bold uppercase tracking-widest">No spirits/drinks found</p>
                </div>
              ) : viewMode === 'grid' ? (
                  ...
                  {filteredProducts.map((product, idx) => {
                    ...
                    return (
                      <button
                        ...
                        className={`group relative overflow-hidden rounded-xl border p-2 flex flex-col text-left hover:scale-[1.03] active:scale-[0.97] aspect-square transition-all duration-300 ${isInCart ? 'shadow-2xl shadow-black/40 ring-1 ring-offset-1 ring-[#E8A838]/20' : 'hover:shadow-lg'}`}
                      >
                        <div style={{ height: '2px', background: `linear-gradient(90deg, ${accent.color}AA, transparent)` }} className="w-full absolute top-0 left-0" />
                        
                        <div className="absolute right-[-4px] bottom-[-4px] text-[48px] opacity-5 pointer-events-none select-none filter blur-[1px]">
                          {watermark}
                        </div>
                        ...
                        <div className="flex-1 flex flex-col z-10 w-full">
                          <div className="flex justify-between items-start mb-2">
                            ...
                            {isOutOfStock ? (
                              ...
                              <span className="text-[7px] font-semibold text-emerald-400 bg-emerald-400/10 px-1.5 py-0.5 rounded border border-emerald-400/20">{stock}</span>
                            ) : cartQty > 0 ? (
                          ...
                          {product.variants && product.variants.length > 0 && (
                            <p className="text-[7px] text-[#E8A838] font-black uppercase mt-1">
                              {product.variants.length} Peg Sizes
                            </p>
                          )}
                            ...
                            {product.variants && product.variants.length > 0 ? (
                                ...
                                {product.variants.map((v, vIdx) => {
                                  ...
                                  return (
                                    <button 
                                      ...
                                      className={`py-3.5 text-[8px] font-black uppercase tracking-widest text-white transition-all active:scale-95 bg-amber-600 hover:bg-amber-700 ${isLastOdd ? 'col-span-2' : ''} border-r border-b border-white/10`}
                                    >
                                      {v.name}
                                    </button>
                                  );
                              ...
                              <div className="flex w-full">
                                <button 
                                  ...
                                  className="flex-1 bg-amber-500 hover:bg-amber-600 text-white text-[9px] font-black py-3.5 uppercase tracking-wider"
                                >
                                  Half
                                </button>
                                <button 
                                  onClick={(e) => { e.stopPropagation(); addToCart(product, 'Full'); }}
                                  className="flex-1 bg-amber-700 hover:bg-amber-800 text-white text-[9px] font-black py-3.5 uppercase tracking-wider border-l border-white/10"
                                >
                                  Full
                                </button>
                              </div>
                  ...
                  {filteredProducts.map((product, idx) => {
                    ...
                    return (
                      <button 
                        ...
                        style={{
                          backgroundColor: isInCart ? accent.bg : 'rgba(255,255,255,0.02)',
                          borderColor: isInCart ? accent.border : 'rgba(255,255,255,0.05)',
                        }}
                        className="w-full flex items-center gap-3 p-2.5 rounded-xl border text-left cursor-pointer transition-all hover:bg-white/[0.04]"
                      >
                        <div style={{ backgroundColor: accent.bg, borderColor: accent.border }} className="w-10 h-10 rounded-lg flex items-center justify-center border shrink-0">
                          <ProductIcon productName={product.name} size={18} style={{ color: accent.color }} />
                        </div>
                        <div className="flex-1 min-w-0">
                          <h3 className="text-xs font-bold text-white uppercase truncate">{product.name}</h3>
                          <p className="text-[9px] text-slate-500 font-medium uppercase mt-0.5">{product.variants?.length || 0} peg sizes</p>
                        </div>
                        <div className="text-right shrink-0">
                          <p style={{ color: accent.color }} className="text-xs font-black">₹{product.sellingPrice.toFixed(0)}</p>
                        </div>
                      </button>
                    );
                  })}
          ...
          <>
            ...
            <div className="px-3 py-1.5 flex items-center justify-between gap-3 bg-white/[0.01] border-b border-white/[0.05] backdrop-blur-md flex-shrink-0 z-10">
              <div className="flex items-center gap-2">
                ...
                {/* Brand Logo & Title */}
                <div className="flex items-center gap-2 flex-shrink-0">
                  <div className="w-8 h-8 rounded-lg bg-[#D4956A]/10 border border-[#D4956A]/20 flex items-center justify-center shadow-[0_0_12px_rgba(212,149,106,0.1)]">
                    <Coffee size={16} color="#D4956A" />
                  </div>
                  <div>
                    <h1 className="text-xs font-black text-[#D4956A] tracking-widest leading-none uppercase">
                      {terminalMode === 'RESTAURANT' ? 'Restaurant POS' : terminalMode === 'BAR' ? 'Bar POS' : 'Cafe POS'}
                    </h1>
                    <p className="text-[7px] text-slate-500 tracking-wider font-semibold uppercase mt-0.5">
                      {terminalMode === 'RESTAURANT' ? 'Classic Dining' : terminalMode === 'BAR' ? 'Premium Bar' : 'Specialty Coffee'}
                    </p>
                  </div>
                </div>
                  ...
                  <div className="flex items-center gap-0.5 p-0.5 rounded-lg border bg-white/[0.02] border-white/[0.05] flex-shrink-0 ml-2">
                    {property?.restaurantPosEnabled !== false && (
                      <button
                        ...
                        className={`px-2 py-1 rounded text-[9px] font-black uppercase tracking-wider transition-all ${
                          terminalMode === 'RESTAURANT'
                            ? 'bg-[#D4956A]/20 border border-[#D4956A]/30 text-[#D4956A] shadow-sm'
                            : 'text-slate-400 hover:bg-[#D4956A]/10 hover:text-[#D4956A]'
                        }`}
                      >
                        🍽 Restaurant
                      </button>
              ...
              <div className="flex-1 flex items-center gap-2 max-w-xl">
                <div className="relative flex-1 group">
                  <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-[#D4956A] transition-colors" size={14} />
                  <input 
                    type="text" 
                    placeholder="Search drinks, snacks..."
                    value={search}
                    onChange={(e) => setSearch(e.target.value)}
                    className="w-full bg-[#1A130F] border border-white/5 focus:border-[#D4956A]/40 pl-9 pr-3 py-1.5 rounded-xl outline-none text-slate-200 placeholder:text-slate-600 font-bold text-xs"
                  />
                </div>
            ...
            <div className="px-3 py-1.5 overflow-x-auto no-scrollbar flex gap-2 border-b border-white/[0.03] bg-white/[0.005]">
              <button 
                ...
                className={`flex-shrink-0 px-4 py-1.5 rounded-full text-xs font-semibold cursor-pointer white-space-nowrap transition-all ${
                  selectedCategory === 'all' 
                    ? 'border border-[#D4956A]/40 bg-[#D4956A]/15 text-[#D4956A] shadow-[0_0_12px_rgba(212,149,106,0.08)]' 
                    : 'border border-white/5 bg-[#1A130F] text-slate-400 hover:text-white'
                }`}
              >
                ☕ All Items
              </button>
              ...
              {mainCategories.map((cat, idx) => {
                const catLower = cat.name.toLowerCase();
                const iconKey  = Object.keys(CAFE_CAT_ICON_MAP).find(k => catLower.includes(k)) || 'default';
                const emoji    = CAFE_CAT_ICON_MAP[iconKey] || '☕';
                const isSelected = selectedCategory === cat.id;
                ...
                return (
                  <button 
                    ...
                    style={{
                      borderColor: isSelected ? accent.border : 'rgba(255,255,255,0.05)',
                      backgroundColor: isSelected ? accent.bg : '#1A130F',
                      color: isSelected ? accent.color : 'rgba(255,255,255,0.4)',
                    }}
                    className="flex-shrink-0 display-flex items-center gap-1.5 px-4 py-1.5 rounded-full text-xs font-semibold cursor-pointer white-space-nowrap transition-all border"
                  >
            ...
            {/* CAFE SUBCATEGORY PILLS */}
            {currentSubCategories.length > 0 && (
              <div className="px-3 py-1 overflow-x-auto no-scrollbar flex gap-1.5 border-b border-white/[0.02] bg-black/15">
                <button 
                  onClick={() => setSelectedSubCategory('all')} 
                  className={`flex-shrink-0 px-3 py-1 rounded-full text-[10px] font-black uppercase transition-all ${
                  ...
                  return (
                    <button 
                      ...
                      className={`flex-shrink-0 px-3 py-1 rounded-full text-[10px] font-black uppercase transition-all ${
                        isSelected 
                          ? 'border border-white/20 bg-white/10 text-white' 
                          : 'border border-white/5 bg-white/2 text-slate-400 hover:text-white'
                      }`}
                    >
                      {subCat.name}
                    </button>
            ...
            {/* CAFE PRODUCTS GRID / LIST */}
            <div className="flex-1 overflow-y-auto p-3 scrollbar-thin">
              {filteredProducts.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center gap-3 opacity-20 py-12">
                  <Coffee size={44} className="text-slate-500" />
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5">
                            <h3 className="text-xs font-bold text-white uppercase truncate">{product.name}</h3>
                            {popular && <span className="text-[6px] font-black text-yellow-500 bg-yellow-500/10 border border-yellow-500/20 px-1 rounded-full">★ BEST</span>}
                          </div>
                          <p className="text-[9px] text-slate-500 font-medium uppercase mt-0.5">{product.variants?.length || 0} size options</p>
                        </div>
                        <div className="text-right shrink-0">
                          <p style={{ color: accent.color }} className="text-xs font-black">₹{product.sellingPrice.toFixed(0)}</p>
                        </div>
          ...
          <>
            ...
            <div className="px-3 py-1 flex items-center justify-between gap-3">
               <div className="flex items-center gap-2">
                 ...
                 {/* ── POS MODE SWITCHER ── */}
                 {(((property?.restaurantPosEnabled !== false ? 1 : 0) + (property?.barPosEnabled ? 1 : 0) + (property?.cafePosEnabled ? 1 : 0)) > 1) && (
                   <div className={`flex items-center gap-1 p-1 rounded-xl border ${theme === 'dark' ? 'bg-white/[0.03] border-white/[0.07]' : 'bg-slate-100/80 border-slate-200'} flex-shrink-0`}>
                     {/* Restaurant POS */}
                     {property?.restaurantPosEnabled !== false && (
                       <button
                         onClick={terminalMode !== 'RESTAURANT' ? () => router.push(`${p}/billing`) : undefined}
                         ...
                         className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] uppercase tracking-wider transition-all ${
                           terminalMode === 'RESTAURANT'
                             ? theme === 'dark'
                               ? 'bg-pos-primary/20 border border-pos-primary/40 text-pos-primary shadow-[0_2px_10px_rgba(0,0,0,0.3)] font-black cursor-default'
                               : 'bg-white border border-pos-primary/30 text-pos-primary shadow-sm font-black cursor-default'
                             : theme === 'dark'
                               ? 'text-slate-400 hover:bg-pos-primary/10 hover:text-pos-primary border border-transparent font-semibold cursor-pointer'
                               : 'text-slate-500 hover:bg-slate-50 hover:text-pos-primary border border-transparent font-semibold cursor-pointer'
                         }`}
                       >
                     ...
                     {property?.barPosEnabled && (
                       <button
                         ...
                         className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] uppercase tracking-wider transition-all ${
                           terminalMode === 'BAR'
                             ? theme === 'dark'
                               ? 'bg-amber-500/20 border border-amber-500/40 text-amber-400 shadow-[0_2px_10px_rgba(0,0,0,0.3)] font-black cursor-default'
                               : 'bg-white border border-amber-500/30 text-amber-600 shadow-sm font-black cursor-default'
                             : theme === 'dark'
                               ? 'text-slate-400 hover:bg-amber-500/10 hover:text-amber-400 border border-transparent font-semibold cursor-pointer'
                               : 'text-slate-500 hover:bg-amber-50 hover:text-amber-600 border border-transparent font-semibold cursor-pointer'
                         }`}
                       >
                         <span className="text-[13px]">🍺</span>
                         <span>Bar POS</span>
                     ...
                     {/* Cafe POS tab ── only when enabled */}
                     {property?.cafePosEnabled && (
                       <button
                         onClick={terminalMode !== 'CAFE' ? () => router.push(`${p}/cafe-pos`) : undefined}
                         ...
                         className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] uppercase tracking-wider transition-all ${
                           terminalMode === 'CAFE'
                             ? theme === 'dark'
                               ? 'bg-[#D4956A]/20 border border-[#D4956A]/40 text-[#D4956A] shadow-[0_2px_10px_rgba(0,0,0,0.3)] font-black cursor-default'
                               : 'bg-white border border-amber-500/30 text-amber-700 shadow-sm font-black cursor-default'
                             : theme === 'dark'
                               ? 'text-slate-400 hover:bg-[#D4956A]/10 hover:text-[#D4956A] border border-transparent font-semibold cursor-pointer'
                               : 'text-slate-500 hover:bg-amber-50 hover:text-amber-700 border border-transparent font-semibold cursor-pointer'
                         }`}
                       >
                         <span className="text-[13px]">☕</span>
                         <span>Cafe POS</span>
               ...
               <div className="flex-1 flex items-center gap-2 max-w-xl">
                  <div className="relative flex-1 group">
                    ...
                    <input 
                      type="text" 
                      placeholder={terminalMode === 'BAR' ? 'Search drinks, spirits...' : terminalMode === 'CAFE' ? 'Search drinks, snacks...' : 'Search menu items...'}
                      value={search}
                      onChange={(e) => setSearch(e.target.value)}
            ...
            <div className="px-3 py-0.5 overflow-x-auto no-scrollbar flex gap-2">
               ...
               {categories.filter(c => (c as any).menuType === terminalMode || (terminalMode === 'RESTAURANT' && !(c as any).menuType)).slice(0, 15).map((cat, idx) => {
                ...
                <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-7 2xl:grid-cols-8 gap-2">
                   {/* Show Combos ONLY if 'combos' category is selected */}
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex justify-between items-start w-full relative z-10">
                            <span className="text-[8px] font-black uppercase tracking-widest" style={{ color: cardColor.text, opacity: theme === 'light' ? 0.7 : 0.6 }}>COMBO DEAL</span>
                            <div className="text-right">
                              <p className="text-[7px] font-black uppercase opacity-40 leading-none mb-0.5">Price</p>
                              <span className="text-[14px] font-black tracking-tight leading-none" style={{ color: cardColor.text }}>₹{combo.price.toFixed(0)}</span>
                            </div>
                          </div>

                          {/* Middle: Name */}
                          <div className="flex-1 flex items-center w-full relative z-10 py-0.5">
                            <h3 className="text-[18px] leading-[1.0] font-black break-words w-full uppercase overflow-hidden line-clamp-2" style={{ color: cardColor.text, fontFamily: 'var(--font-bebas-neue)' }}>{combo.name}</h3>
                          </div>

                          {/* Bottom Row */}
                          <div className="w-full relative z-10 flex items-end justify-between">
                             <div>
                                <p className="text-[7px] font-bold uppercase tracking-widest opacity-60" style={{ color: cardColor.text }}>{combo.items?.length || 0} PRODUCTS</p>
                                <p className="text-[7px] font-black uppercase tracking-tighter opacity-40" style={{ color: cardColor.text }}>SPECIAL DEAL</p>
                             </div>
                    ...
                    {filteredProducts.map((product, idx) => {
                      ...
                      const transitionStr = (prop: string) => `${prop} ${isColored ? '1.2s' : '0.3s'} cubic-bezier(0.25,0.9,0.4,1) ${staggerDelay}`;
                      ...
                      return (
                        <div
                          ...
                          className={`group relative rounded-xl p-1.5 flex flex-col text-left overflow-hidden hover:scale-[1.03] active:scale-[0.97] aspect-square w-full ${isInCart ? 'shadow-2xl ring-2 ring-offset-2' : 'hover:shadow-xl'} transition-all duration-300 cursor-pointer`}
                        >
                          <div className="absolute inset-0 rounded-lg opacity-0 group-hover:opacity-100 pointer-events-none" style={{ background: `radial-gradient(circle at top right, ${isColored ? cardColor.border : '#fff'}30 0%, transparent 70%)`, transition: 'opacity 0.3s ease' }} />
                          ...
                          <div className="flex justify-between items-start w-full relative z-10">
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className="text-[8px] font-black uppercase tracking-widest" style={{ color: isColored ? cardColor.text : darkText, transition: transitionStr('color'), opacity: theme === 'light' ? 0.7 : 0.6 }}>HSN {product.hsnCode || '2106'}</span>
                            </div>
                            <div className="text-right">
                              <p className="text-[7px] font-black uppercase opacity-40 leading-none mb-0.5">Price</p>
                              <span className="text-[14px] font-black tracking-tight leading-none" style={{ color: isColored ? cardColor.text : darkText, transition: transitionStr('color') }}>₹{product.sellingPrice.toFixed(0)}</span>
                            </div>
                          </div>

                          {/* Middle: Product Name */}
                          <div className="flex-1 flex items-center w-full relative z-10 py-0.5">
                            <h3 className="text-[18px] leading-[1.0] font-black break-words w-full uppercase overflow-hidden line-clamp-2" style={{ color: isColored ? cardColor.text : darkText, transition: transitionStr('color'), fontFamily: 'var(--font-bebas-neue)' }}>
                              {(() => {
                          ...
                          <div className={`w-full relative z-10 pt-0.5 border-t ${theme === 'light' ? 'border-black/10' : 'border-white/5'}`}>
                            <div className="flex justify-between items-end h-4">
                              <div className="flex flex-col">
                                <span className="text-[7px] font-black uppercase tracking-widest truncate max-w-[100px]" style={{ color: isColored ? cardColor.text : darkText, opacity: theme === 'light' ? 0.8 : 0.5 }}>{categories.find(c => c.id === product.categoryId)?.name || 'N/A'}</span>
                                <span className="text-[7px] font-black uppercase tracking-tighter" style={{ color: isColored ? cardColor.text : darkText, opacity: theme === 'light' ? 0.9 : 0.6 }}>GST {product.taxRate || 5}%</span>
                              </div>
                              <div className="opacity-30" style={{ color: isColored ? cardColor.text : darkText }}><ProductIcon productName={product.name} categoryName={categories.find(c => c.id === product.categoryId)?.name} size={12} /></div>
                            </div>
                          </div>
                          ...
                          {((product.variants?.length ?? 0) > 0 || (product as any).halfPrice > 0) && (
                            <div className="absolute inset-x-0 bottom-0 translate-y-[100%] group-hover:translate-y-0 transition-transform duration-300 z-20">
                              {product.variants && product.variants.length > 0 ? (
                                ...
                                <div className="flex w-full">
                                  <button onClick={(e) => { e.stopPropagation(); addToCart(product, 'Half'); }} className="flex-1 bg-amber-500 hover:bg-amber-600 text-white text-[11px] font-black py-4 uppercase tracking-wider">Half</button>
                                  <button onClick={(e) => { e.stopPropagation(); addToCart(product, 'Full'); }} className="flex-1 bg-pos-primary hover:bg-pos-primary-dark text-white text-[11px] font-black py-4 uppercase tracking-wider border-l border-white/10">Full</button>
                                </div>
                              )}
                            </div>
                          )}
                          ...
                          {isInCart && (
                            <div className="absolute top-2 right-2 z-30 pointer-events-none">
                              <div className="text-white p-0.5 rounded-full shadow-md" style={{ backgroundColor: cardColor.border }}>
                      ...
                      >
                        ...
                        <div className="text-right flex flex-col items-end">
                          ...
                          <div className="mt-2">
                            ...
                            ) : ((product as any).menuType === terminalMode || (terminalMode === 'RESTAURANT' && !(product as any).menuType)) && (product as any).halfPrice > 0 && (
                              <div className="flex flex-col gap-1.5">
                                <button onClick={(e) => { e.stopPropagation(); addToCart(product, 'Half'); }} className="px-4 py-1.5 bg-amber-500 text-white rounded-xl text-[9px] font-black hover:bg-amber-600 transition-all active:scale-95 shadow-lg shadow-amber-500/20 uppercase">HALF</button>
                                <button onClick={(e) => { e.stopPropagation(); addToCart(product, 'Full'); }} className="px-4 py-1.5 bg-pos-primary text-white rounded-xl text-[9px] font-black hover:bg-pos-primary-dark transition-all active:scale-95 shadow-lg shadow-pos-primary/20 uppercase">FULL</button>
                              </div>
                            )}
                          </div>
                          {isInCart ? (
          ...
          >
            ...
            {activeOrders.map((order) => {
              ...
              return (
                <button
                  key={order.tableId}
                  onClick={() => router.push(`${p}${basePath}?tableId=${order.tableId}&tableName=${order.tableName}&orderId=${order.orderId}`)}
                  ...
                  style={{
                    backgroundColor: isCurrentTable
                      ...
                      : (theme === 'dark' ? '#222' : '#fff'),
                    borderColor: isCurrentTable ? statusColor : (theme === 'dark' ? '#2e2e2e' : '#e8e8e8'),
                    borderWidth: '2px',
                    borderStyle: 'solid',
      ...
      <div className={`w-[460px] ${theme === 'dark' ? 'bg-[#1a1a1a] border-white/5' : 'bg-white border-pos-primary/20'} border-l flex flex-col h-full sticky top-0 shadow-2xl z-10 transition-all duration-300`}>
        ...
        <div className="p-3 pb-1 flex flex-col gap-2">
           <div className="flex items-center justify-between">
              <div>
                 <h2 className={`text-lg font-black ${theme === 'dark' ? 'text-slate-100' : 'text-slate-800'} tracking-tight`}>
                   ...
                   <div key={item.cartItemId} className={`group relative overflow-hidden ${theme === 'dark' ? 'bg-[#1a1a1a] border-[#2a2a2a] shadow-lg' : 'bg-white border-slate-200 shadow-md'} rounded-xl p-1.5 border transition-all duration-300 mb-1`}>
                     <div className="flex flex-col gap-0.5 relative z-10">
                       ...
                       <div className="grid grid-cols-2 gap-1.5">
                         {((item as any).menuType === terminalMode || (terminalMode === 'RESTAURANT' && !(item as any).menuType)) && (!item.variants || item.variants.length === 0) && (item as any).halfPrice > 0 && (
                           <>
                             <button
                       ...
                       {/* Hot/Cold toggle for CAFE layout */}
                       {themeLayout === 'CAFE' && canToggleTemp(item.name) && (
                         <div className="mt-1 flex justify-start">
                           <button
                             onClick={(e) => { e.stopPropagation(); toggleTemp(item.cartItemId); }}
                             title={item.temp === 'HOT' ? 'Switch to Iced' : 'Switch to Hot'}
                             className={`flex items-center gap-1.5 px-2 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all duration-300 ${
                               item.temp === 'HOT'
