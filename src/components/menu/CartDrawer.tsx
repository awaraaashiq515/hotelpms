...
import { X, Minus, Plus, CheckCircle, Wallet, CreditCard, QrCode, Smartphone } from 'lucide-react';
                    ...
                    {cart.map((item) => (
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between bg-white dark:bg-slate-900 rounded-2xl p-1 border border-slate-100 dark:border-slate-800 shadow-sm">
                          <button onClick={() => removeFromCart(item.id)} className="w-8 h-8 flex items-center justify-center text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/30 rounded-xl transition-colors"><Minus size={14} /></button>
                          <span className="font-black text-xs tabular-nums text-slate-900 dark:text-white">{item.quantity}</span>
                          <button onClick={() => addToCart(item)} className="w-8 h-8 flex items-center justify-center text-pos-accent hover:bg-pos-accent/10 rounded-xl transition-colors"><Plus size={14} /></button>
                        </div>
                      </motion.div>
                    ))}
                  </div>

                  {/* Payment method selection removed as per user request. Orders are placed as 'Pay Later' by default. */}
