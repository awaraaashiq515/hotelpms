...
export default function RoomStatusBoardView() {
  ...
  const fetchBoardData = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (json.success && json.data) {
        ...
        if (!silent) toast.error(json.message || 'Failed to load Room Status Board');
      }
    } catch (e: unknown) {
      if (!silent) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className={`px-3 h-9 rounded-xl border text-xs font-black flex items-center gap-1.5 transition-all ${
              autoRefresh
          ...
          <button
            ...
            title="Refresh Board"
            className={`w-9 h-9 rounded-xl bg-slate-900/80 hover:bg-slate-800 border border-white/10 text-slate-300 hover:text-white flex items-center justify-center transition-all ${
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-3">
        {[
          ...
          <div
            key={s.label}
            className={`rounded-2xl bg-gradient-to-br ${s.color} border ${s.border} p-3.5 backdrop-blur-md shadow-lg`}
          >
      ...
      <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-3 p-3.5 rounded-2xl bg-slate-900/80 border border-white/10 backdrop-blur-md shadow-xl">
        {/* Search */}
        <div className="relative flex-1 min-w-[240px]">
          ...
          <input
            ...
            className="w-full h-10 pl-10 pr-4 bg-slate-800/80 border border-white/10 rounded-xl text-xs text-white placeholder:text-slate-500 focus:outline-none focus:border-indigo-500 transition-colors"
          />
        </div>
        ...
        {floors
          ...
          .map((fl) => {
            ...
            return (
              <div
                key={fl}
                className="rounded-2xl bg-slate-900/60 border border-white/10 p-5 backdrop-blur-md shadow-xl"
              >
                {/* Floor Header Bar */}
                  ...
                  {floorRooms.map((room) => {
                    ...
                    return (
                      <div
                        ...
                        className={`p-4 rounded-2xl border ${cardBg} cursor-pointer transition-all hover:scale-[1.02] hover:shadow-xl flex flex-col justify-between group`}
                      >
                        <div>
                          ...
                          {/* Occupied Guest Info */}
                          {room.activeGuest ? (
                            <div className="p-2.5 rounded-xl bg-slate-950/50 border border-white/5 space-y-1 mb-2">
                            ...
                            <div className="p-2.5 rounded-xl bg-slate-950/30 border border-white/5 text-[10px] text-slate-500 mb-2">
      ...
      {filteredRooms.length === 0 && (
        <div className="text-center py-16 rounded-2xl border border-white/5 bg-slate-900/40">
