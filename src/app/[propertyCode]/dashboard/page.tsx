    ...
    <div className="space-y-6 lg:space-y-8 h-full pb-10 lg:pb-0">
      ...
      {!showWizard && (
        <>
          ...
          {/* ── Plan & Subscription Status Card (RESTAURANTS_ADMIN only) ── */}
          {isAdmin && (() => {
            const packageEndDate = session?.packageEndDate;
            const packageName = session?.packageName || 'Standard Plan';
            const subscriptionStatus = session?.subscriptionStatus || 'ACTIVE';
            const packageFeaturesList: string[] = session?.packageFeatures || [];
            ...
            if (packageEndDate) {
              ...
              expiryDateString = expiry.toLocaleDateString('en-IN', { year: 'numeric', month: 'short', day: 'numeric' });
            ...
            const barColor = isExpired ? 'bg-red-500' : isNearExpiry ? 'bg-amber-400' : 'bg-emerald-500';
            const borderColor = isExpired ? 'border-red-500/20' : isNearExpiry ? 'border-amber-500/20' : 'border-indigo-500/10';

            return (
              <div className={`bg-white dark:bg-slate-900 rounded-3xl border-2 ${borderColor} dark:border-slate-800 p-6 shadow-sm`}>
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  {/* Left: Plan Info */}
                  <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-2xl shrink-0 ${isExpired ? 'bg-red-50 dark:bg-red-950/20' : isNearExpiry ? 'bg-amber-50 dark:bg-amber-950/20' : 'bg-indigo-50 dark:bg-indigo-950/20'}`}>
                      ⚡
                    </div>
                    <div>
                      <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Active Subscription</p>
                      <h3 className="text-base font-black text-slate-900 dark:text-white uppercase tracking-tight mt-0.5">{packageName}</h3>
                      <div className="flex items-center gap-2 mt-1">
                        <span className={`text-[9px] font-black px-2 py-0.5 rounded-full uppercase tracking-widest border ${
                          isExpired ? 'text-red-600 bg-red-50 border-red-100 dark:bg-red-950/20 dark:border-red-900/50' :
                          isNearExpiry ? 'text-amber-600 bg-amber-50 border-amber-100 dark:bg-amber-950/20 dark:border-amber-900/50' :
                          'text-emerald-600 bg-emerald-50 border-emerald-100 dark:bg-emerald-950/20 dark:border-emerald-900/50'
                        }`}>
                          {isExpired ? 'EXPIRED' : isNearExpiry ? 'EXPIRING SOON' : subscriptionStatus}
                        </span>
                        <span className="text-[10px] text-slate-400 dark:text-slate-500 font-bold">
                          {packageFeaturesList.length} Features Active
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-4 shrink-0">
                    <div className="text-right">
                      ...
                      <p className="text-sm font-black text-slate-900 dark:text-white mt-0.5">{expiryDateString}</p>
                      <p className={`text-[10px] font-bold mt-0.5 ${isExpired ? 'text-red-500' : isNearExpiry ? 'text-amber-500' : 'text-emerald-500'}`}>
                        {isExpired ? 'Access Restricted' : `${daysRemaining} days remaining`}
                      </p>
                    </div>
                    <button
                      onClick={() => window.location.href = '/settings'}
                      className="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-md shadow-indigo-500/20 active:scale-95"
                    >
                      Manage Plan
                    </button>
                  </div>
                ...
                {packageEndDate && (
                  <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 space-y-2">
                    <div className="flex justify-between text-[9px] font-black text-slate-400 uppercase tracking-widest">
                      ...
                      <span>{isExpired ? '0 days left' : `${daysRemaining} / ${totalDays} days`}</span>
                    </div>
                    <div className="w-full h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                      <div className={`h-full ${barColor} rounded-full transition-all duration-700`} style={{ width: `${progressPercent}%` }} />
                    </div>
                  </div>
                )}
              </div>
