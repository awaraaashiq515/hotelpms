...
export default function BillingPage() {
  ...
  useEffect(() => {
    ...
    const key = `pos_layout_billing_${propertyCode.toLowerCase()}`;
    const saved = localStorage.getItem(key) as PosTheme | null;
    console.log(`[POS Billing Page] Read key: ${key} = ${saved}`);
  ...
  if (!layout) {
    return (
      <div className="h-screen bg-slate-950 flex flex-col items-center justify-center gap-3 text-white">
        ...
        <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Loading POS...</span>
      </div>
    );
  }
