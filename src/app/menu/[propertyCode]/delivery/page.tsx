...
interface CartItem {
  id: string;           // productId or productId_variantId
  productId: string;
  name: string;
  sellingPrice: number;
  quantity: number;
...
export default function HomeDeliveryPage() {
  ...
  const addToCartWithVariant = (product: any, variant?: any) => {
  ...
  const addToCart = (product: any) => {
    // If product has variants, open the picker instead
  ...
  const removeFromCart = (cartId: string) => {
    setCart(prev => {
      const existing = prev.find(i => i.id === cartId);
      if (existing && existing.quantity > 1) return prev.map(i => i.id === cartId ? { ...i, quantity: i.quantity - 1 } : i);
              ...
              <div key={cat.id} id={`cat-${cat.id}`} className="scroll-mt-52">
                ...
                <div className="space-y-3.5">
                  {cat.products.map((product: any) => {
                    // Sum all cart items belonging to this product (any variant)
                    const productCartItems = cart.filter(i => i.productId === product.id || i.id === product.id);
                    const productCartQty = productCartItems.reduce((s, i) => s + i.quantity, 0);
                    ...
                    return (
                      ...
                      >
                          ...
                          {hasVariants && (
                            <div className="flex flex-wrap gap-1.5 mt-2">
                              {product.variants.slice(0, 3).map((v: any) => (
                                <span key={v.id} className="text-[9px] font-black uppercase tracking-wide px-2 py-0.5 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400">
                                  {v.name} · ₹{v.sellingPrice ?? product.sellingPrice}
                                </span>
                              ))}
                        ...
                        <div className="shrink-0 pl-1">
                          {hasVariants ? (
                            // Variant product: always show ADD button that opens picker
                            <div className="flex flex-col items-center gap-1.5">
                              <button
                                onClick={() => setVariantPickerProduct(product)}
                              ...
                              {productCartQty > 0 && (
                                <span className="text-[9px] font-black text-indigo-400 bg-indigo-500/10 border border-indigo-500/20 rounded-full w-5 h-5 flex items-center justify-center">
                                  {productCartQty}
                                </span>
                              )}
                            </div>
                            ...
                            <div className="flex items-center gap-2 bg-indigo-950/40 rounded-2xl px-2 py-1.5 border border-indigo-900/40 shadow-inner">
                              ...
                              <span className="text-xs font-black text-indigo-400 w-5 text-center">{productCartQty}</span>
                              <button 
        ...
        {variantPickerProduct && (
          <>
            ...
            <motion.div
              ...
              className="fixed bottom-0 left-0 right-0 z-50 bg-slate-900 border-t border-slate-800 rounded-t-[2.5rem] p-6 shadow-2xl max-w-md mx-auto max-h-[80vh] flex flex-col"
            >
              {/* Header */}
              <div className="flex items-start justify-between mb-5">
                <div className="flex items-start gap-3">
                  {variantPickerProduct.image && (
                    <img src={variantPickerProduct.image} alt={variantPickerProduct.name} className="w-12 h-12 rounded-xl object-cover border border-slate-800 shrink-0" />
                  )}
                  <div>
                    <h3 className="text-sm font-black text-white uppercase tracking-tight leading-tight">{variantPickerProduct.name}</h3>
                    <p className="text-[9px] font-black text-indigo-400 uppercase tracking-widest mt-1">Choose a Size / Variant</p>
                  </div>
                </div>
                <button onClick={() => setVariantPickerProduct(null)} className="w-8 h-8 rounded-xl bg-slate-950 border border-slate-800 flex items-center justify-center text-slate-400 hover:text-white transition-all cursor-pointer shrink-0">
                  <X size={15} />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto space-y-3 no-scrollbar pr-1">
                {variantPickerProduct.variants.map((variant: any) => {
                  const cartId = `${variantPickerProduct.id}_${variant.id}`;
                  const cartEntry = cart.find(i => i.id === cartId);
                  const price = variant.sellingPrice ?? variantPickerProduct.sellingPrice;
                  return (
                    <div key={variant.id} className="flex items-center justify-between gap-4 bg-slate-950/50 border border-slate-800/80 rounded-2xl px-4 py-3.5">
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-black text-white uppercase tracking-tight">{variant.name}</p>
                        <p className="text-sm font-black text-indigo-400 mt-0.5">₹{price}</p>
                      </div>
                      {cartEntry ? (
                        <div className="flex items-center gap-2.5 bg-indigo-950/40 rounded-2xl px-2.5 py-1.5 border border-indigo-900/40 shadow-inner shrink-0">
                          <button
                          ...
                          <span className="text-xs font-black text-indigo-400 w-5 text-center">{cartEntry.quantity}</span>
                          <button
                            onClick={() => addToCartWithVariant(variantPickerProduct, variant)}
                            className="w-7 h-7 rounded-xl bg-indigo-600 flex items-center justify-center text-white active:scale-95 transition-transform shadow-md"
                          >
                            <Plus size={11} />
                          </button>
                        ...
                        <button
                          onClick={() => addToCartWithVariant(variantPickerProduct, variant)}
                          className="h-9 px-5 rounded-2xl bg-indigo-600 hover:bg-indigo-500 active:scale-95 text-white text-[10px] font-black uppercase tracking-widest shadow-md transition-all cursor-pointer shrink-0"
                        >
                          Add
                        </button>
                      )}
              ...
              <button
                onClick={() => setVariantPickerProduct(null)}
                className="mt-5 w-full py-4 bg-slate-800 hover:bg-slate-700 text-white rounded-2xl text-xs font-black uppercase tracking-widest transition-all cursor-pointer"
              >
                Done — View Cart
              </button>
            </motion.div>
