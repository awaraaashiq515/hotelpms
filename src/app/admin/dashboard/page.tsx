    ...
    <div className="relative min-h-screen p-2 md:p-0 isolate overflow-hidden dark:bg-slate-950">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <h1 className="text-4xl font-black text-slate-900 dark:text-white tracking-tight flex items-baseline gap-3 transition-colors">
            System Dashboard
            <span className="text-sm font-bold bg-rose-50 dark:bg-rose-900/30 px-3 py-1 rounded-full uppercase tracking-widest border border-rose-100 dark:border-rose-800" style={{color:'#e8a0a0'}}>Live</span>
          </h1>
          ...
          <Card key={i} className={`p-0 border-none shadow-xl ${card.shadow} dark:shadow-none hover:shadow-2xl hover:-translate-y-1 transform-gpu transition-shadow,transform duration-200 group overflow-hidden relative isolate dark:bg-slate-900/50 dark:border dark:border-slate-800`}>
            ...
            <div className="p-8">
              <div className="flex justify-between items-start mb-6 font-sans">
                <div className={`p-4 bg-gradient-to-br ${card.color} rounded-2xl shadow-lg ring-4 ring-white dark:ring-slate-800 transform-gpu`}>
                  <card.icon className="text-white" size={24} />
                </div>
                <div className="flex flex-col items-end">
                  <span className="text-[10px] font-black text-emerald-600 bg-emerald-50 dark:bg-emerald-900/20 dark:text-emerald-400 px-2 py-1 rounded-lg flex items-center gap-1 border border-emerald-100 dark:border-emerald-800 transition-colors">
              ...
              <div className="space-y-1">
                ...
                <h3 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight tabular-nums transition-colors">{card.value}</h3>
              </div>
            </div>
            {/* Decorative element */}
            <div className={`absolute -bottom-6 -right-6 w-24 h-24 bg-gradient-to-br ${card.color} opacity-[0.05] dark:opacity-[0.1] rounded-full blur-2xl group-hover:scale-150 transform-gpu transition-transform duration-500`} />
          </Card>
        ))}
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <Card className="lg:col-span-2 p-10 border-none shadow-xl shadow-slate-100 dark:shadow-none dark:bg-slate-900/50 dark:border dark:border-slate-800 relative overflow-hidden">
          <div className="absolute top-0 right-0 p-8 opacity-[0.03] dark:opacity-[0.05] pointer-events-none">
            <Activity size={200} className="text-slate-900 dark:text-white" />
          </div>
          <div className="flex justify-between items-center mb-10 relative z-10">
            <div>
              <h3 className="text-lg font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-3 transition-colors">
                <div className="h-2 w-2 rounded-full animate-ping" style={{backgroundColor:'#e8a0a0'}} />
                Live Network Health
              </h3>
              <p className="text-xs text-slate-400 font-medium mt-1">Real-time request distribution across clusters</p>
            </div>
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full" style={{backgroundColor:'#e8a0a0'}} />
                <span className="text-[9px] font-black text-slate-400 uppercase">Traffic</span>
              </div>
              <div className="flex items-center gap-2">
          ...
          <div className="h-[280px] w-full flex items-end justify-between gap-1 pt-4 relative z-10 transition-colors">
        ...
        <Card className="p-10 border-none shadow-xl shadow-slate-100 dark:shadow-none flex flex-col h-full overflow-hidden bg-white dark:bg-slate-900 text-slate-900 dark:text-white relative transition-colors duration-500">
          ...
          <div className="flex justify-between items-start mb-8">
            <h3 className="text-sm font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">Live Stream</h3>
            <div className="flex items-center gap-2 px-3 py-1 bg-rose-50 dark:bg-white/10 rounded-full border border-rose-100 dark:border-white/5">
              ...
              <span className="text-[9px] font-black uppercase tracking-tighter text-emerald-600 dark:text-emerald-400">Active</span>
            </div>
          </div>
          <div className="space-y-8 flex-1 overflow-y-auto no-scrollbar pr-2">
            {(stats?.recentOrders || []).map((order: any, idx: number) => (
              <div key={order.id} className="flex items-center gap-5 group">
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-sm font-black text-white shrink-0 shadow-lg rotate-3 group-hover:rotate-0 transition-all ${
                  ['bg-rose-400', 'bg-emerald-500', 'bg-amber-500', 'bg-rose-600'][idx % 4]
                }`}>
                  {order.property.name.charAt(0)}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-black truncate uppercase tracking-tighter group-hover:text-rose-400 transition-colors">{order.property.name}</p>
                  <div className="flex items-center gap-2 mt-1">
                    <div className="w-1 h-1 rounded-full bg-white/20" />
                    <p className="text-[9px] text-slate-500 font-bold uppercase truncate tracking-widest leading-none">{order.servedBy?.fullName || 'POS Staff'}</p>
                  </div>
                </div>
            ...
            {(!stats?.recentOrders || stats.recentOrders.length === 0) && (
              <div className="flex flex-col items-center justify-center py-20 text-center opacity-40">
                ...
                <p className="text-[10px] font-bold uppercase tracking-widest leading-loose text-white/60">Awaiting Signal...<br/>Monitoring Global Sync</p>
              </div>
            )}
          </div>
          <button className="mt-8 py-4 w-full bg-white/5 border border-white/10 rounded-2xl text-[10px] font-black uppercase tracking-[0.3em] hover:bg-white/10 transition-colors hover:text-white group">
            <span className="group-hover:tracking-[0.4em] transition-all">Access Full Logs</span>
          </button>
        </Card>
      </div>
    </div>
