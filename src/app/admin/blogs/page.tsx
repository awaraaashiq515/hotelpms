    ...
    <div className="space-y-8 pb-24">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-500 font-medium">Create and manage professional blog posts for your website.</p>
        </div>
        <button
          onClick={() => {
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {blogs.map((blog) => (
            <div key={blog.id} className="bg-white rounded-[32px] shadow-sm border border-slate-100 overflow-hidden flex flex-col group hover:shadow-xl transition-all duration-500">
              ...
              <div className="p-8 flex-1 flex flex-col">
                ...
                <p className="text-slate-500 text-sm mb-6 line-clamp-2 font-medium opacity-80">{blog.excerpt || 'No excerpt provided...'}</p>
                <div className="mt-auto flex justify-end items-center pt-6 border-t border-slate-50 gap-3">
                   <button onClick={() => { setCurrentBlog(blog); setShowModal(true); }} className="p-3 bg-slate-50 text-slate-900 hover:bg-slate-900 hover:text-white rounded-2xl transition-all">
                    <Edit2 size={18} />
                  </button>
                  <button onClick={() => handleDelete(blog.id)} className="p-3 bg-slate-50 text-red-500 hover:bg-red-500 hover:text-white rounded-2xl transition-all">
                    <Trash2 size={18} />
                  </button>
                </div>
              </div>
          ...
          {blogs.length === 0 && (
            <div className="col-span-full py-24 text-center bg-slate-50 rounded-[40px] border-2 border-dashed border-slate-200">
               <p className="text-slate-400 font-bold uppercase tracking-widest opacity-60">No blogs found. Start writing your first story!</p>
            </div>
          )}
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-6">
          <div className="bg-white rounded-[40px] w-full max-w-4xl p-10 animate-in zoom-in-95 duration-300 shadow-2xl overflow-y-auto max-h-[90vh]">
            <div className="flex justify-between items-center mb-8">
              <h2 className="text-3xl font-bold uppercase tracking-tighter">{currentBlog.id ? 'Edit' : 'Create'} Blog Post</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-10">
              ...
              <div className="md:col-span-8 space-y-6">
                <div>
                  ...
                  <input
                    type="text"
                    placeholder="e.g. 5 Hidden Gems in Manali You Must Visit"
                    value={currentBlog.title}
                    onChange={e => setCurrentBlog({ ...currentBlog, title: e.target.value })}
                    className="w-full p-4 bg-slate-50 rounded-2xl border-none focus:ring-2 focus:ring-pos-primary outline-none transition-all font-bold text-lg"
                  />
                </div>

                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Short Excerpt</label>
                  <textarea
                    placeholder="Provide a brief summary for the listing page..."
                    value={currentBlog.excerpt}
                    onChange={e => setCurrentBlog({ ...currentBlog, excerpt: e.target.value })}
                    className="w-full p-4 bg-slate-50 rounded-2xl border-none focus:ring-2 focus:ring-pos-primary outline-none h-24 transition-all resize-none font-medium text-sm"
                  />
                </div>

                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Full Content (HTML/Rich Text)</label>
                  <textarea
                    placeholder="Write your story here..."
                    value={currentBlog.content}
                    onChange={e => setCurrentBlog({ ...currentBlog, content: e.target.value })}
                    className="w-full p-6 bg-slate-50 rounded-[32px] border-none focus:ring-2 focus:ring-pos-primary outline-none h-[400px] transition-all font-mono text-sm leading-relaxed"
                  />
                </div>
              </div>
              ...
              <div className="md:col-span-4 space-y-6">
                <div>
                  ...
                  <input
                    ...
                    className="w-full p-3 bg-slate-50 rounded-xl border-none focus:ring-2 focus:ring-pos-primary outline-none text-xs font-mono"
                  />
                </div>
                ...
                <div>
                  ...
                  <select 
                    ...
                    className="w-full p-3 bg-slate-50 rounded-xl border-none focus:ring-2 focus:ring-pos-primary outline-none text-xs font-bold appearance-none bg-[url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20width%3D%2220%22%20height%3D%2220%22%20viewBox%3D%220%200%2020%2020%22%20fill%3D%22none%22%20xmlns%3D%22http%3A//www.w3.org/2000/svg%22%3E%3Cpath%20d%3D%22M5%207.5L10%2012.5L15%207.5%22%20stroke%3D%22%2394A3B8%22%20stroke-width%3D%221.66667%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22/%3E%3C/svg%3E')] bg-[length:20px_20px] bg-[right_10px_center] bg-no-repeat"
                  >
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Featured Image</label>
                  <div className="space-y-4">
                    <div className="aspect-square bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200 overflow-hidden flex items-center justify-center relative">
                      {currentBlog.imageUrl ? (
                        <img src={currentBlog.imageUrl} alt="" className="w-full h-full object-cover" />
                      ) : (
                        <div className="text-slate-300"><Plus size={32} /></div>
                      )}
                      {modalLoading && (
                ...
                <div className="flex items-center gap-4 bg-slate-50 p-4 rounded-2xl">
                  ...
                  />
                  <label htmlFor="isActive" className="text-[10px] font-bold text-slate-700 uppercase tracking-widest cursor-pointer">Published</label>
                </div>
              </div>
            </div>

            <div className="flex justify-end gap-4 pt-10 border-t border-slate-50 mt-10">
              <button 
