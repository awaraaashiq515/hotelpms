...
export default function PropertiesPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-0 overflow-hidden border-2 border-slate-200/60 dark:border-slate-800 shadow-xl dark:shadow-none rounded-2xl">
        {loading ? (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/80 dark:bg-slate-900/50 border-b-2 border-slate-100 dark:border-slate-800">
                  ...
                  <th className="px-6 py-5 text-[10px] font-black uppercase text-slate-500 dark:text-slate-400 tracking-[0.2em]">Unique Code</th>
                  ...
                  <tr key={prop.id} className="hover:bg-pos-primary/5 dark:hover:bg-slate-800/50 transition-all group">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-bold text-slate-900 dark:text-white">{prop.name}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-slate-700 dark:text-slate-300">{prop._count?.users || 0} Users</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button 
                          onClick={() => handleSwitchProperty(prop.id, prop.name)}
                          className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-emerald-600 hover:text-white transition-all shadow-sm"
                          title="Enter Property"
                        >
                          <ArrowRightCircle size={14} /> Enter
                        </button>
                        <button 
                          onClick={() => handleOpenModal(prop)}
                          className="p-2 text-slate-400 dark:text-slate-500 hover:text-pos-primary dark:hover:text-pos-primary/80 hover:bg-pos-primary/5 dark:hover:bg-slate-800 rounded-lg transition-all"
                          title="Edit Branch"
                        >
                          <Edit2 size={16} />
                        </button>
                  ...
                  <tr>
                    <td colSpan={6} className="px-6 py-20 text-center">
                       <div className="flex flex-col items-center gap-2 opacity-30">
                         <AlertCircle size={40} />
                         <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">No branches found. Create your first branch above.</p>
                       </div>
                    </td>
                  </tr>
                )}
