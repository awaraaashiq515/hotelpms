...
export default function DayClosingPage() {
  ...
  const inputCls = 'w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm font-bold focus:border-pos-primary/40 focus:bg-white outline-none transition-all';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-gray-900 uppercase tracking-tight">Day Closing</h1>
      ...
      {/* Status Bar */}
      {loading ? (
        <div className="bg-white rounded-2xl border border-gray-100 p-8 text-center text-xs font-bold text-gray-300 uppercase">Loading shift...</div>
      ) : shift ? (
        ...
        <div className="bg-gray-50 border border-dashed border-gray-200 rounded-2xl p-10 flex flex-col items-center justify-center gap-4 text-gray-400">
        ...
        {tabs.map(({ id, label, icon: Icon }) => (
          <button
            ...
            className={`flex items-center gap-2 px-5 py-3 text-[11px] font-black uppercase tracking-widest whitespace-nowrap border-b-2 transition-all ${
              tab === id ? 'border-pos-primary text-pos-primary' : 'border-transparent text-gray-400 hover:text-gray-700'
            }`}
          >
            <Icon size={14} />
            {label}
      ...
      {tab === 'current' && (
        <div className="space-y-4">
          {!shift ? (
            <div className="text-center text-xs font-bold text-gray-400 uppercase py-10">Open a shift to see live data</div>
          ) : (
            <>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                  <div className="p-5 border-b border-gray-50 flex items-center gap-3">
                    <div className="p-2 bg-red-50 rounded-xl"><ArrowDownCircle size={18} className="text-red-500" /></div>
                    <div>
                      <h3 className="font-black text-sm text-gray-900">Cash Withdrawals</h3>
                  ...
                  {shift.withdrawals.length === 0 ? (
                    <div className="p-8 text-center text-xs font-bold text-gray-300 uppercase">No withdrawals</div>
                  ) : (
                    <div className="divide-y divide-gray-50">
                      {shift.withdrawals.map(w => (
                        <div key={w.id} className="px-5 py-3 flex items-center justify-between">
                          <div>
                            <p className="text-xs font-bold text-gray-900">₹{w.amount.toLocaleString('en-IN')}</p>
                            <p className="text-[10px] text-gray-400 font-medium">{w.reason || 'No reason'}</p>
                          </div>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                  <div className="p-5 border-b border-gray-50 flex items-center gap-3">
                    <div className="p-2 bg-green-50 rounded-xl"><ArrowUpCircle size={18} className="text-green-500" /></div>
                    <div>
                      <h3 className="font-black text-sm text-gray-900">Cash Top-Ups</h3>
                      <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">
                  ...
                  {shift.topUps.length === 0 ? (
                    <div className="p-8 text-center text-xs font-bold text-gray-300 uppercase">No top-ups</div>
                  ) : (
                    <div className="divide-y divide-gray-50">
                      {shift.topUps.map(t => (
                        <div key={t.id} className="px-5 py-3 flex items-center justify-between">
                          <div>
                            <p className="text-xs font-bold text-gray-900">₹{t.amount.toLocaleString('en-IN')}</p>
                            <p className="text-[10px] text-gray-400 font-medium">{t.reason || 'No reason'}</p>
                          </div>
                          <p className="text-[9px] text-gray-400 font-bold">
      ...
      {tab === 'withdrawal' && (
        <div className="max-w-md">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 space-y-5">
            <div>
              <h3 className="font-black text-lg text-gray-900 uppercase tracking-tight">Cash Withdrawal</h3>
              <p className="text-xs text-gray-400 font-bold uppercase tracking-widest mt-1">Record cash taken out of the till</p>
            </div>
            {!shift && (
      ...
      {tab === 'topup' && (
        <div className="max-w-md">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 space-y-5">
            <div>
              <h3 className="font-black text-lg text-gray-900 uppercase tracking-tight">Cash Top-Up</h3>
              <p className="text-xs text-gray-400 font-bold uppercase tracking-widest mt-1">Add extra cash to the till</p>
            </div>
            {!shift && (
              <div className="bg-amber-50 border border-amber-100 rounded-xl p-4 text-amber-700 text-xs font-black uppercase flex items-center gap-2">
                <AlertTriangle size={16} /> No active shift. Open a shift first.
      ...
      {tab === 'history' && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="p-5 border-b border-gray-50 flex items-center justify-between">
            <div>
              <h3 className="font-black text-sm text-gray-900 uppercase">Closing History</h3>
              <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Last 30 day closings</p>
            </div>
            <button onClick={fetchHistory} className="p-2.5 text-gray-400 hover:text-pos-primary hover:bg-pos-primary/10 rounded-xl transition-colors">
              <RefreshCw size={16} />
            </button>
          </div>
              ...
              <thead>
                <tr className="border-b border-gray-50 bg-gray-50/50">
                  {['Date', 'Shift', 'Cashier', 'Invoices', 'Total Sales', 'Cash', 'Card', 'UPI', 'Expected', 'Actual', 'Variance'].map(h => (
                    <th key={h} className="px-4 py-3 text-left text-[9px] font-black text-gray-400 uppercase tracking-[0.12em] whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {historyLoading ? (
                  <tr><td colSpan={11} className="py-16 text-center text-xs font-bold text-gray-300 uppercase">Loading...</td></tr>
                ) : history.length === 0 ? (
                  <tr><td colSpan={11} className="py-16 text-center text-xs font-bold text-gray-300 uppercase">No closing records yet</td></tr>
                ) : history.map(c => (
                  <tr key={c.id} className="border-b border-gray-50 hover:bg-gray-50/30 transition-colors">
                    ...
                    <td className="px-4 py-3.5 text-xs text-gray-600 font-bold">{c.shift?.shiftNo || '—'}</td>
                    <td className="px-4 py-3.5 text-xs text-gray-500">{c.shift?.cashierName || c.closedBy || '—'}</td>
                    <td className="px-4 py-3.5 text-xs font-bold text-gray-700">{c.invoiceCount}</td>
                    <td className="px-4 py-3.5 text-sm font-black text-gray-900">₹{c.totalSales.toLocaleString('en-IN')}</td>
                    <td className="px-4 py-3.5 text-xs font-bold text-gray-700 flex items-center gap-1"><Banknote size={11} className="text-emerald-500" />₹{c.cashSales.toFixed(0)}</td>
                    <td className="px-4 py-3.5 text-xs font-bold text-gray-700"><CreditCard size={11} className="inline text-blue-500 mr-1" />₹{c.cardSales.toFixed(0)}</td>
                    <td className="px-4 py-3.5 text-xs font-bold text-gray-700"><Smartphone size={11} className="inline text-purple-500 mr-1" />₹{c.upiSales.toFixed(0)}</td>
                    <td className="px-4 py-3.5 text-xs font-bold text-blue-700">₹{c.expectedCash.toFixed(2)}</td>
                    <td className="px-4 py-3.5 text-xs font-bold text-gray-900">₹{c.actualCash.toFixed(2)}</td>
                    <td className="px-4 py-3.5">
                      <span className={`text-xs font-black ${c.varianceAmount === 0 ? 'text-gray-400' : c.varianceAmount > 0 ? 'text-pos-primary' : 'text-red-500'}`}>
