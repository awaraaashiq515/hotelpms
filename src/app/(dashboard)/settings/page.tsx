...
const BarPosSettingsForm = () => {
  ...
  useEffect(() => {
    fetch('/api/setup/properties/current')
      .then(r => r.json())
      .then(data => {
        if (data.success) { setProperty(data.data); setBarPosEnabled(!!data.data.barPosEnabled); }
        setLoading(false);
      })
      .catch(() => setLoading(false));
  }, []);
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(`/api/setup/properties/${property.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ barPosEnabled }),
      });
  ...
  return (
    <div style={{ borderLeft: '6px solid #f97316', borderRadius: '1rem' }}><Card className="p-8 overflow-hidden border-l-0">
