...
import {
  ...
  Hash, Package, User, Truck, Printer
} from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {summaryCards.map((card) => (
          <div
            key={card.label}
            className="bg-white rounded-[24px] border border-gray-100 p-6 shadow-sm hover:shadow-xl hover:shadow-pos-primary/5 transition-all group overflow-hidden relative"
          >
            ...
            <div className="flex items-center justify-between mb-4 relative z-10">
              <div className={`w-12 h-12 rounded-2xl ${card.bg} border flex items-center justify-center group-hover:scale-110 transition-transform shadow-sm`}>
                <card.icon size={22} />
              </div>
            </div>
            <p className="text-3xl font-black text-gray-900 tracking-tight relative z-10 mb-1">{card.value}</p>
            <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-gray-500 relative z-10">{card.label}</p>
          </div>
        ))}
      </div>
        ...
        <div className="p-4 md:p-6 border-b border-gray-100 bg-gray-50/50 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="relative w-full md:w-80">
            ...
            <input
              ...
              className="pl-12 pr-4 py-3 bg-white border border-gray-200 rounded-2xl text-sm w-full focus:ring-4 focus:ring-pos-primary/10 focus:border-pos-primary transition-all font-semibold outline-none shadow-sm"
            />
          </div>
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0 no-scrollbar">
            ...
            {datePreset === 'custom' && (
              <input
                ...
                className="px-4 py-2 bg-white border border-gray-200 rounded-xl text-xs font-black text-gray-700 outline-none focus:ring-2 focus:ring-pos-primary/20 shadow-sm"
              />
            )}
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 w-12"></th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">Bill Details</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">Order Info</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">Staff/Table</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 text-right">Totals</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 text-right w-24">Actions</th>
              </tr>
            </thead>
              ...
              {loading ? (
                Array.from({ length: 5 }).map((_, i) => (
                  <tr key={i} className="animate-pulse">
                    <td className="px-6 py-6"><div className="w-6 h-6 bg-gray-100 rounded-lg"></div></td>
                    <td className="px-6 py-6"><div className="w-24 h-4 bg-gray-100 rounded mb-2"></div><div className="w-16 h-3 bg-gray-50 rounded"></div></td>
                    <td className="px-6 py-6"><div className="w-20 h-4 bg-gray-100 rounded mb-2"></div><div className="w-12 h-3 bg-gray-50 rounded"></div></td>
                    <td className="px-6 py-6"><div className="w-20 h-4 bg-gray-100 rounded mb-2"></div><div className="w-12 h-3 bg-gray-50 rounded"></div></td>
                    <td className="px-6 py-6 text-right"><div className="w-20 h-4 bg-gray-100 rounded ml-auto mb-2"></div><div className="w-12 h-3 bg-gray-50 rounded ml-auto"></div></td>
                    <td className="px-6 py-6"><div className="w-10 h-10 bg-gray-100 rounded-xl ml-auto"></div></td>
                  </tr>
                ))
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-24 text-center">
                    <div className="flex flex-col items-center gap-4 text-gray-300">
                      ...
                      <div>
                        <p className="text-sm font-black uppercase tracking-widest text-gray-400">No bills found</p>
                        <p className="text-[11px] font-bold text-gray-400 mt-1">Adjust your filters to see results</p>
                      </div>
                    </div>
                  </td>
                </tr>
                ...
                filteredBills.map((bill) => (
                  <React.Fragment key={bill.id}>
                    <tr className={`transition-colors group ${expandedId === bill.id ? 'bg-pos-primary/5' : 'hover:bg-gray-50'}`}>
                      <td className="px-6 py-4">
                        <button 
                          onClick={() => setExpandedId(expandedId === bill.id ? null : bill.id)}
                          className={`w-8 h-8 rounded-xl flex items-center justify-center transition-all ${
                            expandedId === bill.id ? 'bg-pos-primary/20 text-pos-primary' : 'bg-gray-100 text-gray-400 group-hover:bg-gray-200 group-hover:text-gray-600'
                          }`}
                        >
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm font-black text-gray-900 tracking-tight">{bill.orderNo}</p>
                      ...
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest ${
                          bill.orderType === 'DINE_IN' ? 'bg-pos-primary/10 text-pos-primary border border-pos-primary/20'
                            : bill.orderType === 'DELIVERY' ? 'bg-orange-50 text-orange-600 border border-orange-100'
                            : 'bg-emerald-50 text-emerald-600 border border-emerald-100'
                        }`}>
                          {bill.orderType.replace('_', ' ')}
                        </span>
                        <p className="text-[11px] font-bold text-gray-500 mt-1.5">{new Date(bill.createdAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}</p>
                      </td>
                      <td className="px-6 py-4">
                        <div className="space-y-1">
                          {bill.tableNo && (
                            <p className="text-xs font-black text-gray-800">Table: {bill.tableNo}</p>
                          )}
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex flex-col items-end">
                          <span className="text-lg font-black text-gray-900 tracking-tight">₹{bill.grandTotal.toFixed(2)}</span>
                      ...
                      <td className="px-6 py-4 text-right">
                        <button
                          ...
                          className="w-10 h-10 rounded-xl bg-white border border-gray-200 text-gray-600 flex items-center justify-center hover:bg-pos-primary/10 hover:text-pos-primary hover:border-pos-primary/20 transition-all shadow-sm ml-auto"
                          title="Print Receipt"
                        >
                          <Printer size={18} />
                        </button>
                    ...
                    {expandedId === bill.id && (
                      <tr className="bg-gray-50/50">
                        <td colSpan={6} className="px-6 py-4 border-b border-gray-100 shadow-inner">
                          <div className="bg-white rounded-2xl border border-gray-100 p-4 ml-14 max-w-3xl shadow-sm">
                              ...
                              {bill.items?.map((item, idx) => (
                                <div key={item.id} className="flex items-center justify-between text-xs py-1.5 border-b border-gray-50 last:border-0">
                                  ...
                                  <div className="flex items-center gap-8">
                                    <span className="text-gray-500 font-medium">{item.quantity} x ₹{item.unitPrice.toFixed(2)}</span>
                                    <span className="font-black text-gray-900 w-16 text-right">₹{item.totalAmount.toFixed(2)}</span>
                                  </div>
                                </div>
                              ))}
                            </div>
