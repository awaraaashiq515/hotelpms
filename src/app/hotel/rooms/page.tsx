...
import RoomStatusBoardView from '@/components/hotel/rooms/RoomStatusBoardView';
...
function HotelRoomsContent() {
  ...
  const [activeTab, setActiveTab] = useState<'board' | 'rooms' | 'types'>(
    tabParam === 'board' ? 'board' : tabParam === 'types' ? 'types' : 'board'
  );

  useEffect(() => {
    if (tabParam === 'board' || tabParam === 'rooms' || tabParam === 'types') {
      setActiveTab(tabParam);
    }
  }, [tabParam]);

  const handleTabChange = (tab: 'board' | 'rooms' | 'types') => {
    setActiveTab(tab);
    const params = new URLSearchParams(window.location.search);
    params.set('tab', tab);
    router.replace(`/hotel/rooms?${params.toString()}`, { scroll: false });
  };
  ...
  if (loading && rooms.length === 0 && roomTypes.length === 0) {
    return (
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800/80 pb-5">
        <div className="space-y-1">
          <span className="flex items-center gap-1.5 text-xs font-black uppercase tracking-widest text-indigo-400">
            <Sparkles size={12} /> Rooms & Types Section
          </span>
        ...
        {/* Tab Controls */}
        <div className="flex bg-slate-950 p-1.5 rounded-2xl border border-slate-800/80 self-start overflow-x-auto max-w-full">
          <button
            onClick={() => handleTabChange('board')}
            className={`px-4 sm:px-5 py-2.5 rounded-xl font-bold text-xs transition-all flex items-center gap-2 whitespace-nowrap ${
              activeTab === 'board' 
                ? 'bg-indigo-600 text-white shadow-md' 
                : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            <MapPin size={14} /> Room Status Board
          </button>
          <button
      ...
      {/* ── 2. Rooms Grid Inventory Tab View ── */}
      {activeTab === 'rooms' && (
        <div className="space-y-6 animate-in fade-in duration-200">
          {/* Dynamic Metrics Strips */}
