  ...
  const fetchLogs = async (monthStr: string = selectedMonth) => {
    ...
    try {
      ...
      if (data.success && data.data) {
        // New API returns { present: [...], absent: [...] }
        // This page needs the present (attendance) records only
        const presentRecords = Array.isArray(data.data)
          ? data.data  // fallback if old format
          : (data.data.present ?? []);

        // Flatten: each grouped record → individual log entry for location display
        // We expand grouped records back to individual session detail
        const flatLogs: AttendanceLog[] = presentRecords
          .filter((r: any) => r.clockIn) // skip absent/no-record
          .map((r: any) => ({
            id: r.id,
            clockIn: r.clockIn,
            clockOut: r.clockOut,
