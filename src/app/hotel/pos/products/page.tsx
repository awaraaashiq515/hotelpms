...
import { Filter, Package, Tag, Edit, Trash2, Plus, ChevronDown, Download, Sparkles, Upload, Check, AlertCircle } from 'lucide-react';
import { PageHeader } from '@/components/shared/page-header';
import { SearchToolbar } from '@/components/shared/search-toolbar';
import { DataTable } from '@/components/shared/data-table';
import { Button } from '@/components/ui/Button';
import { productsApi, Product } from '@/lib/api/products';
import { Building2, Leaf, Flame } from 'lucide-react';
...
export default function HotelPosProductsPage() {
    ...
    },
    {
      ...
      cell: (row: Product) => (
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-slate-800 border border-slate-700 flex items-center justify-center overflow-hidden text-orange-400">
            {row.image ? (
              <img src={row.image} alt={row.name} className="w-full h-full object-cover" />
            ) : (
              <ProductIcon productName={row.name} categoryName={row.category?.name} size={16} className="opacity-60" />
            )}
          </div>
          <div className="flex flex-col gap-0.5">
            ...
            <div className="flex items-center gap-1.5 opacity-70">
              ...
              <span className="text-[10px] font-bold uppercase tracking-widest text-slate-300">{row.category?.name || 'Uncategorized'}</span>
              {row.hsnCode && (
                <>
              ...
              {row.taxRate !== null && row.taxRate !== undefined && (
                <>
                  <span className="mx-1 text-slate-600">•</span>
                  <span className="text-[10px] font-bold uppercase tracking-widest text-orange-400">GST: {row.taxRate}%</span>
                </>
              )}
              {row.taxType && (
                <>
                  ...
                  <span className={`text-[10px] font-bold uppercase tracking-widest px-1.5 py-0.5 rounded ${
                    row.taxType === 'INCLUSIVE' ? 'bg-indigo-500/20 text-indigo-300' :
                    row.taxType === 'EXCLUSIVE' ? 'bg-blue-500/20 text-blue-300' :
    ...
    },
    {
      ...
      cell: (row: Product) => (
        <div className="flex flex-col">
          <span className="text-sm font-black text-white">₹{row.sellingPrice.toFixed(2)}</span>
          {row.costPrice > 0 && (
            <span className="text-[10px] text-slate-400 font-medium">Cost: ₹{row.costPrice.toFixed(2)}</span>
          )}
        </div>
      ),
      width: '150px'
      ...
      cell: (row: Product) => (
        <div className="flex items-center gap-2">
          <button
            ...
            className="p-2 hover:bg-slate-800 rounded-lg text-slate-400 hover:text-pos-primary transition-colors cursor-pointer"
            title="Edit"
          >
            <Edit size={16} />
          </button>
  ...
  const comboColumns = [
    {
      ...
      cell: (row: Combo) => (
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-orange-500/10 border border-orange-500/20 flex items-center justify-center overflow-hidden text-orange-400">
            {row.image ? (
              <img src={row.image} alt={row.name} className="w-full h-full object-cover" />
            ) : (
              <Package size={20} />
    ...
    },
    {
      header: 'Price',
      cell: (row: Combo) => (
        <span className="text-sm font-black text-white">₹{row.price.toFixed(2)}</span>
      ),
      width: '150px'
    },
    {
      ...
      cell: (row: Combo) => (
        <div className="flex items-center gap-2">
          <button
            ...
            className="p-2 hover:bg-slate-800 rounded-lg text-slate-400 hover:text-orange-400 transition-colors cursor-pointer"
            title="Edit"
          >
            <Edit size={16} />
          </button>
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        ...
        subtitle="Manage restaurant dishes, pricing, bar spirits, cafe items, and combos"
        showBack
        backUrl="/hotel"
        actions={
          <div className="flex gap-2 flex-wrap items-center">
            ...
            <div className="relative group">
              <Button
                ...
                variant="secondary"
                className="font-black text-[9px] tracking-widest px-3 py-2 rounded-lg border border-emerald-500/30 bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 flex items-center gap-1 disabled:opacity-60 cursor-pointer"
              >
                {seedingMenu
              ...
              <div className="absolute top-full mt-1 right-0 hidden group-hover:flex flex-col bg-slate-900 border border-slate-800 rounded-xl shadow-xl z-50 overflow-hidden min-w-[160px]">
                <button onClick={() => handleSeedMenu('RESTAURANT')} className="px-4 py-2.5 text-[10px] font-black uppercase tracking-widest text-indigo-400 hover:bg-slate-800 text-left cursor-pointer">Restaurant Only</button>
                <button onClick={() => handleSeedMenu('BAR')} className="px-4 py-2.5 text-[10px] font-black uppercase tracking-widest text-amber-400 hover:bg-slate-800 text-left cursor-pointer">Bar Only</button>
                <button onClick={() => handleSeedMenu('ALL')} className="px-4 py-2.5 text-[10px] font-black uppercase tracking-widest text-emerald-400 hover:bg-slate-800 text-left cursor-pointer">All (Restaurant + Bar)</button>
              </div>
            </div>
            ...
            <Button
              ...
              variant="secondary"
              className={`font-black text-[9px] tracking-widest px-3 py-2 rounded-lg border flex items-center gap-1 cursor-pointer ${
            ...
            <Button
              ...
              variant="secondary"
              className="font-black text-[9px] tracking-widest px-3 py-2 rounded-lg border border-amber-500/30 bg-amber-500/10 text-amber-300 hover:bg-amber-500/20 cursor-pointer"
            >
              TAX SETTINGS
            </Button>
            ...
            {propertyDetails?.restaurantPosEnabled !== false && (
              <Button
                ...
                className="bg-indigo-600 hover:bg-indigo-700 text-white font-black text-[9px] tracking-widest px-3 py-2 rounded-lg shadow-lg shadow-indigo-500/20 cursor-pointer"
              >
            ...
            {propertyDetails?.barPosEnabled && (
              <Button
                ...
                className="bg-amber-500 hover:bg-amber-600 text-slate-950 font-black text-[9px] tracking-widest px-3 py-2 rounded-lg shadow-lg shadow-amber-500/20 cursor-pointer"
              >
                <Plus size={14} className="mr-1.5" />
                + BAR
              </Button>
            ...
            {propertyDetails?.cafePosEnabled && (
              <Button
                ...
                className="bg-[#D2691E] hover:bg-[#B55A1A] text-white font-black text-[9px] tracking-widest px-3 py-2 rounded-lg shadow-lg shadow-orange-500/20 cursor-pointer"
              >
                <Plus size={14} className="mr-1.5" />
                + CAFE
              </Button>
            ...
            <Button
              ...
              className="bg-orange-600 hover:bg-orange-700 text-white font-black text-[9px] tracking-widest px-3 py-2 rounded-lg shadow-lg shadow-orange-500/20 cursor-pointer"
            >
              <Plus size={14} className="mr-1.5" />
              + COMBO
            </Button>
      ...
      {seedToast && (
        <div className="mx-auto mb-2 flex items-center gap-3 px-5 py-3 rounded-2xl bg-emerald-600 text-white text-[11px] font-black uppercase tracking-widest shadow-lg shadow-emerald-500/30 animate-in slide-in-from-top-3 duration-300">
      ...
      <SearchToolbar
        value={search}
        onChange={setSearch}
        placeholder="Search by dish name, SKU or category..."
        actions={
          <div className="flex gap-2 flex-wrap items-center">
            {['SUPER_ADMIN', 'RESTAURANTS_ADMIN'].includes(session?.role) && properties.length > 0 && (
              <div className="relative group">
                <Building2 size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 group-hover:text-pos-primary transition-colors z-10" />
                <select
                  value={selectedPropertyId}
                  onChange={(e) => setSelectedPropertyId(e.target.value)}
                  className="pl-10 pr-8 py-2.5 bg-slate-900 border border-slate-800 rounded-xl text-xs font-bold uppercase tracking-widest text-slate-200 outline-none focus:ring-2 focus:ring-pos-primary/20 focus:border-pos-primary transition-all appearance-none cursor-pointer min-w-[200px]"
                >
                ...
                <ChevronDown size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none transition-transform group-hover:translate-y-[-40%]" />
              </div>
            )}
            <div className="flex items-center gap-1 p-1 bg-slate-900 border border-slate-800 rounded-xl">
              ...
              <button
                onClick={() => setSelectedMenuTypeFilter('AI_SCAN')}
                className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-1 cursor-pointer ${
                  selectedMenuTypeFilter === 'AI_SCAN'
              ...
              {[
                ...
                <label
                  ...
                  className={`flex items-center gap-4 p-4 rounded-2xl border-2 transition-all cursor-pointer ${
                    selectedTaxType === type.id
                      ? 'border-pos-primary bg-pos-primary/10 shadow-sm'
                ...
                >
                  <input
                    ...
                    className="w-5 h-5 text-pos-primary focus:ring-pos-primary border-slate-700"
                  />
                  <div>
                    <p className="font-bold text-sm text-white uppercase tracking-tight">{type.label}</p>
    ...
    try {
      ...
      const mappedCategories = (pythonData.categories || []).map((category: any) => {
        const items = (category.items || []).map((item: any) => {
          ...
          const costPrice = Math.round(sellingPrice * 0.4 * 100) / 100;
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-slate-800 pb-5 mb-6 gap-4">
        ...
        <div className="flex p-1 bg-slate-800 border border-slate-700 rounded-xl shadow-inner">
      ...
      {error && (
        <div className="mb-6 p-4 bg-red-950/20 border border-red-900/50 text-red-400 rounded-2xl flex items-center gap-3 text-sm">
      ...
      {!scannedData && !scanning && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 space-y-6">
            <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest">Scanner Configuration</h3>
            
            <div className="space-y-4">
              <div className="flex items-center justify-between p-4 bg-slate-900 border border-slate-800 rounded-xl">
                <div>
                  <p className="text-[10px] font-black text-white uppercase tracking-widest">Auto-Extract GST Rates</p>
                  <p className="text-[8px] text-slate-400 font-bold uppercase tracking-tight mt-0.5">Assigns tax rates based on menu categories</p>
                </div>
              ...
              <div className="flex items-center justify-between p-4 bg-slate-900 border border-slate-800 rounded-xl">
                <div>
                  <p className="text-[10px] font-black text-white uppercase tracking-widest">Auto-Extract HSN Codes</p>
                  <p className="text-[8px] text-slate-400 font-bold uppercase tracking-tight mt-0.5">Applies standard HSN codes to items</p>
                </div>
                <label className="relative inline-flex items-center cursor-pointer scale-90">
                  <input
                    type="checkbox"
              ...
              <div className="flex flex-col gap-3 p-4 bg-slate-900 border border-slate-800 rounded-xl">
                <div>
                  <p className="text-[10px] font-black text-white uppercase tracking-widest">Scanning Mode</p>
                  <p className="text-[8px] text-slate-400 font-bold uppercase tracking-tight mt-0.5">Choose processing engine</p>
                </div>
                <div className="flex gap-2">
                  <button
                    type="button"
                    onClick={() => setScanMode('semantic')}
                    className={`flex-1 py-2 px-3 text-[10px] font-black rounded-xl uppercase tracking-wider transition-all border cursor-pointer ${
                      scanMode === 'semantic'
          ...
          <div className="flex flex-col items-center justify-center">
            <div
              ...
              className={`w-full h-64 border-2 border-dashed rounded-2xl flex flex-col items-center justify-center p-6 text-center cursor-pointer transition-all duration-300 ${
                dragActive
                  ...
                  : 'border-slate-800 hover:border-pos-primary hover:bg-slate-800/20'
              }`}
            >
              <input
              ...
              <p className="text-xs text-slate-400 mt-2">Supports JPG, PNG, WEBP, and PDF files</p>
              {file && (
                <span className="text-[10px] bg-indigo-950/40 border border-indigo-900 text-indigo-400 px-3.5 py-1.5 rounded-full font-bold uppercase tracking-wider mt-4">
            ...
            {file && (
              <button
                onClick={startScan}
                className="mt-5 w-full bg-gradient-to-r from-indigo-600 to-rose-600 hover:from-indigo-700 hover:to-rose-700 text-white font-black text-xs uppercase tracking-widest py-4 px-6 rounded-2xl shadow-lg transition-all flex items-center justify-center gap-2 cursor-pointer"
              >
                <Sparkles size={16} />
                Start AI Menu scan
              </button>
      ...
      {scanning && (
        <div className="py-16 flex flex-col items-center justify-center space-y-4">
          ...
          <h3 className="text-sm font-black text-white uppercase tracking-wider animate-pulse">
            {scanStatus || 'Processing menu...'}
          </h3>
          <p className="text-[10px] text-slate-400 uppercase font-black tracking-widest">Please wait. Do not close this page.</p>
        </div>
      )}
      ...
      {scannedData && (
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
          <div className="xl:col-span-4 space-y-4">
            <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest">Uploaded Menu Card</h3>
            <div className="border border-slate-800 rounded-2xl overflow-hidden bg-slate-900/50 shadow-inner max-h-[70vh] flex items-center justify-center p-2">
              {imagePreviewUrl ? (
                <img src={imagePreviewUrl} alt="Menu Preview" className="max-w-full max-h-full object-contain rounded-xl" />
              ) : (
                <div className="py-20 text-slate-400 font-bold uppercase text-[10px]">No image preview available</div>
              )}
            </div>
            <div className="text-[9px] font-bold text-slate-400 uppercase tracking-wide bg-slate-800/40 p-3 rounded-xl border border-slate-800">
          ...
          <div className="xl:col-span-8 space-y-6">
            <div className="flex justify-between items-center">
              <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest">Review Extracted Items</h3>
              <span className="text-[9px] font-black uppercase tracking-widest bg-emerald-500/10 text-emerald-400 px-2.5 py-1 rounded-md border border-emerald-500/20">
            ...
            <div className="space-y-6 max-h-[60vh] overflow-y-auto pr-2 no-scrollbar">
              {scannedData.categories?.map((cat: any, catIdx: number) => (
                <div key={catIdx} className="border border-slate-800 rounded-2xl p-5 bg-slate-800/20 shadow-sm space-y-4">
                  <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        className="bg-transparent font-black text-sm uppercase text-white border-b border-transparent hover:border-slate-700 focus:border-indigo-500 focus:outline-none px-1 py-0.5 rounded transition-all max-w-[200px]"
                      />
                    </div>
                    <button
                      onClick={() => addItemToCategory(catIdx)}
                      className="text-[9px] font-black uppercase tracking-widest text-indigo-400 hover:text-indigo-300 flex items-center gap-1 cursor-pointer"
                    >
                  ...
                  <div className="space-y-4">
                    {cat.items?.map((item: any, itemIdx: number) => (
                      <div key={itemIdx} className="bg-slate-900 border border-slate-800 rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow relative">
                        <div className="flex flex-wrap items-center justify-between gap-3">
                          <div className="flex-1 min-w-[200px]">
                            <input
                              ...
                              className="w-full bg-transparent font-black text-xs uppercase text-slate-100 border-b border-transparent hover:border-slate-700 focus:border-indigo-500 focus:outline-none py-0.5 transition-all"
                          ...
                          <div className="flex items-center gap-2">
                            {/* Full Price Input */}
                            <div className="flex flex-col gap-0.5">
                              <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest pl-1">Full Price</span>
                              <div className="relative">
                                ...
                                <input
                                  ...
                                  className="pl-5 pr-2 py-1.5 w-20 bg-slate-800/60 border border-slate-700 rounded-lg text-xs font-black text-white text-right focus:outline-none focus:border-indigo-500"
                                />
                              </div>
                            </div>
                            ...
                            {/* Half Price Input */}
                            <div className="flex flex-col gap-0.5">
                              <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest pl-1">Half Price</span>
                              <div className="relative">
                                <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400 text-[10px] font-bold">₹</span>
                                <input
                                  type="number"
                            ...
                            <button
                              ...
                              className="p-1.5 rounded-lg border border-red-900/30 hover:bg-red-950/40 text-red-400 transition-all cursor-pointer"
                            >
                              <Trash2 size={14} />
                            </button>
                          </div>
                        ...
                        <div className="mt-2.5">
                          <input
                            ...
                            className="w-full bg-transparent text-[10px] text-slate-400 font-bold border-b border-transparent hover:border-slate-700 focus:border-indigo-500 focus:outline-none py-0.5"
                        ...
                        <div className="flex flex-wrap gap-3 items-center mt-3 pt-3 border-t border-slate-800">
                          <div className="flex items-center gap-1.5">
                            <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">HSN</span>
                            <input
                              type="text"
                              value={item.hsnCode || ''}
                              onChange={(e) => updateItemField(catIdx, itemIdx, 'hsnCode', e.target.value)}
                              className="px-2 py-0.5 bg-slate-800 border border-slate-700 rounded text-[9px] font-bold text-slate-300 w-16 text-center"
                            />
                          </div>

                          <div className="flex items-center gap-1.5">
                            <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">GST</span>
                            <select
                              value={item.taxRate || 5}
                              onChange={(e) => updateItemField(catIdx, itemIdx, 'taxRate', parseFloat(e.target.value))}
                              className="px-2 py-0.5 bg-slate-800 border border-slate-700 rounded text-[9px] font-bold text-slate-300 w-16 text-center focus:outline-none focus:border-indigo-500"
                            >
                          ...
                          <span className="text-[9px] font-mono text-slate-400 ml-auto bg-slate-800 px-2 py-0.5 rounded border border-slate-700 italic">
                            SKU: {item.sku}
                          </span>
                        </div>
                      </div>
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-slate-800">
              <button
                ...
                disabled={saving}
                className="px-5 py-2.5 rounded-xl border border-slate-700 text-xs font-black uppercase tracking-widest text-slate-300 hover:bg-slate-800 transition-colors cursor-pointer"
              >
                Scan Again
              </button>
              <button
                onClick={handleSave}
                disabled={saving}
                className="px-6 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-black uppercase tracking-widest shadow-lg transition-colors flex items-center gap-2 cursor-pointer"
              >
                {saving ? (
                  <>
