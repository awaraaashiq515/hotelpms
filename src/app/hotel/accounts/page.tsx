    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          ...
          <div key={i} className="bg-white dark:bg-slate-800 p-4 rounded-[1.5rem] border border-slate-100 dark:border-slate-700 shadow-sm hover:shadow-md transition-all group overflow-hidden relative">
             <div className="absolute top-0 right-0 w-20 h-20 bg-pos-primary/5 rounded-full -mr-8 -mt-8 transition-all group-hover:scale-150" />
             <div className="relative z-10">
                <div className="flex items-center justify-between mb-4">
                   <div className="p-2 bg-slate-50 dark:bg-slate-700 rounded-xl text-slate-400 group-hover:text-pos-primary transition-colors">
                      <stat.icon size={18} />
                   </div>
                 ...
                 <p className="text-base font-bold text-slate-900 dark:text-white mt-1">{stat.value}</p>
             </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {cards.map((card, i) => (
          <Link key={i} href={card.href} className="group">
            <div className={`h-full p-6 rounded-3xl bg-white dark:bg-slate-800 border-2 ${card.borderColor} ${card.hoverBorder} transition-all duration-300 shadow-xl shadow-slate-200/50 dark:shadow-none hover:-translate-y-2 flex flex-col items-start gap-4 relative overflow-hidden`}>
              {/* Icon Section */}
              <div className={`p-4 ${card.bgColor} dark:bg-slate-700 rounded-2xl shadow-inner transition-transform group-hover:scale-110 duration-500`}>
              ...
              {/* Footer Section */}
              <div className="flex items-center gap-2 text-pos-primary font-bold text-[10px] tracking-widest group-hover:gap-4 transition-all">
                Access Module
      ...
      <div className="bg-slate-900 dark:bg-violet-950 p-6 rounded-[2rem] relative overflow-hidden group">
         <div className="absolute inset-0 bg-pos-primary/10 animate-pulse" />
         <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex-1">
               <h2 className="text-lg font-bold text-white tracking-tight mb-1">Professional Restaurant Accounting</h2>
